...
#[structopt(name = "juice_mlp_nn", about = "multi layer perceptron neural network using juice")]
struct Options {
...
fn main() -> io::Result<()> {
    ...
    // net_cfg.add_layer(layer::LayerConfig::new("tanh", layer::LayerType::TanH));
    ...
    classifier_cfg.add_input("network_out", &[options.batch_size, 10]);
    classifier_cfg.add_input("label", &[options.batch_size, 1]);
    // set up nll loss
    let nll_layer_cfg = layers::NegativeLogLikelihoodConfig { num_classes: 10 };
